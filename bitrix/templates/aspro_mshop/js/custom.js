...
$(function() {
	...
	$('.menu_title a').hover(function() {
		var div = $(this).closest('.menu_item_l1');
		
		$(div).find('.menu_title.active').removeClass('active');
		$(div).find('.menu__doplist').hide();
	...
	function fixToBrand() {
		function create() {
			$('.brand__content').fixTo('.content', {
				top: 50
			});
			$('.brand__content').attr('style', 'position: sticky; top: 50px;')	
		}

		function destroy() {
			try {
				$('.brand__content').fixTo('destroy');
				$('.brand__content').attr('style', '')
			} catch (e) {}
		}

		if ($(window).width() > 1000 && !$('.brand__content').hasClass('fixto')) {
			create();
		} else {
			destroy();
		}
