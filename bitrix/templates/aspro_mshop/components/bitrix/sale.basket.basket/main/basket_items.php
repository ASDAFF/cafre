...
<?if($normalCount > 0):?>
	<div class="module-cart">
		...
		<table class="bottom middle <?=($arError["ERROR"] ? 'error' : '');?>" style="display:none;">
			<tfoot>
					...
					<?else:?>
						...
						<!--<td class="basket_fast_order_wrapp last_blockk" colspan="<?=(1 + ($arParams["SHOW_FULL_ORDER_BUTTON"] == "Y" ? 0 : 1))?>">
								<div class="basket_fast_order clearfix">
									<span onclick="oneClickBuyBasket()" class="button big_btn fast_order"><span><?=GetMessage("SALE_ORDER")?></span></span>
						...
						<?if($arParams["SHOW_FULL_ORDER_BUTTON"] == "Y"):?>
							<td class="basket_checkout_wrapp last_blockk" colspan="<?=(1 + ($arParams["SHOW_FAST_ORDER_BUTTON"] == "Y" ? 0 : 1))?>">
                                 ...
                                 <div class="basket_checkout clearfix">
									<a class="button transparent big_btn checkout" data-text="<?=GetMessage("ORDER_START");?>" data-href="<?=$arParams["PATH_TO_ORDER"];?>" href="<?=$arParams["PATH_TO_ORDER"];?>" onclick="checkOut(event);"><span><?=GetMessage("SALE_FAST_ORDER")?></span></a>
