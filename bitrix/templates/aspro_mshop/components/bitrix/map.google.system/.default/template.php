			...
			if (null == window.bGoogleMapScriptsLoaded){
				if (window.google && window.google.maps){
					...
					BX.loadScript(
						...
						function (){
							if (BX.browser.IsIE())
								setTimeout("window.google.load('maps', <?= intval($arParams['GOOGLE_VERSION'])?>, {callback: init_<?echo $arParams['MAP_ID']?>, other_params: 'key=AIzaSyDcRvC__j3tSJLDcKYiPbvg4z75quenpaA&sensor=false&language=<?= LANGUAGE_ID?>'})", 1000);
							else
								google.load('maps', <?echo intval($arParams['GOOGLE_VERSION'])?>, {callback: init_<?echo $arParams['MAP_ID']?>, other_params: 'key=AIzaSyDcRvC__j3tSJLDcKYiPbvg4z75quenpaA&sensor=false&language=<?=LANGUAGE_ID?>'});
						}
					);
				}
			}else{
