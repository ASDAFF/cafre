...
<script type="text/javascript">
	...
	function checkChange() {
			...
			$('.order__message.delivery_order-message').html('<span>Стоимость: <strong>'+$('.delivery_order [name="DELIVERY_ID"]:checked').closest('label').find('.bx_result_price b').text()+'</strong></span>');
		if($('.delivery_order [name="DELIVERY_ID"]:checked').data('free')>0) {
			$('.order__message.delivery_order-message').append('<span>При заказе от '+$('.delivery_order [name="DELIVERY_ID"]:checked').data('free')+' рублей - <strong>бесплатно</strong></span>');
		}
		$('.order__message.delivery_order-message').append("<span>Доставка от <strong>3 рабочих дней</strong></span>");
	}
