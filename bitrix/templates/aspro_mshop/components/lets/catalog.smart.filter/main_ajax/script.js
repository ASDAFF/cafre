...
var regex_d = /(?:f-clear)/gi;
var dd_d=url.match(regex_d);
if(dd_d){
	$('.container > h1').text($('.old-sec-name').text());
}
/*$('.ckeck-a').on('change',function(){
	var oldtext = $('.old-sec-ajax').text();
	var chektext = $(this).text();
	$('.container > h1').text(oldtext+chektext);
});*/
	$.ajax({
		url:url,
		type: "GET",
