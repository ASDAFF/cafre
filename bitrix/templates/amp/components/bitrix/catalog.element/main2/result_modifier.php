...
//расскоментировать код
if($productSlider){
	foreach($productSlider as $i => $arImage){
    /*   if($USER->IsAdmin()) {
         if(strpos($arImage["SRC"], ".gif") !==false ){
             $big_img = array("src" => $arImage["SRC"]);
         }
         else{
            $big_img = CFile::ResizeImageGet($arImage["ID"], array("width" => 340, "height" => 340), BX_RESIZE_IMAGE_PROPORTIONAL, true, array());   
         }
