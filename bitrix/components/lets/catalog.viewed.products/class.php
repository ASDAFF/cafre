...
class CCatalogViewedProductsComponent extends CBitrixComponent
{
	...
	$last_key = end(array_keys($item_bask));
	
		foreach($item_bask as $key => $val){
			$i2++;
		if($key != 0 && ($key == $last_key || $key == $last_key-1 || $key == $last_key-2)){
			$arr_bb[] = (int)$val;
			//array_unshift($map, (int)$val);
		...
		//$count = count($arr_bb);
			...
			{
				if(in_array($viewedProduct["ELEMENT_ID"], $arr_bb)){
				array_unshift($map, $viewedProduct["ELEMENT_ID"]);
				continue;
				}
