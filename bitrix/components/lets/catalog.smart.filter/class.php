	...
	public function makeSmartUrl($url, $apply, $checkedControlId = false)
	{
		...
		if ($apply)
		{
			foreach($this->arResult["ITEMS"] as $PID => $arItem)
			{
				...
				if ($arItem["PRICE"])
				{
					if (strlen($arItem["VALUES"]["MIN"]["HTML_VALUE"]) > 0){
						...
						$smartPart2["from"] = $arItem["VALUES"]["MIN"]["HTML_VALUE"];
					}
					if (strlen($arItem["VALUES"]["MAX"]["HTML_VALUE"]) > 0){
						$smartPart["to"] = $arItem["VALUES"]["MAX"]["HTML_VALUE"];
						$smartPart2["to"] = $arItem["VALUES"]["MAX"]["HTML_VALUE"];
						}
				}
				...
				if ($smartPart2)
				{
					array_unshift($smartPart2, toLower("price-".$arItem["CODE"]));

					$smartParts2[] = $smartPart2;
				}
			}
			...
			foreach($this->arResult["ITEMS"] as $PID => $arItem)
			{
				...
				{
					if ($arItem["CODE"])
						array_unshift($smartPart2, toLower($arItem["CODE"]));
					else
						array_unshift($smartPart2, $arItem["ID"]);
