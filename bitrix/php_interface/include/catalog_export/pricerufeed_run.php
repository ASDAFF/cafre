...
if (empty($arRunErrors))
{
	...
	fwrite($fp, "\n".'<!DOCTYPE priceru_feed SYSTEM "shops.dtd">'."\n");
	fwrite($fp, '<priceru_feed date="'.date("Y-m-d H:i").'">'."\n");
	fwrite($fp, '<shop>'."\n");
...
/*					$minPrice = $arPrice['DISCOUNT_PRICE'];
			...
			$strTmpOff.= "<url>".$usedProtocol.$ar_iblock['SERVER_NAME'].htmlspecialcharsbx($arAcc["~DETAIL_PAGE_URL"]).(strstr($arAcc['DETAIL_PAGE_URL'], '?') === false ? '?' : '&amp;')."utm_source=price_ru&amp;utm_medium=click&amp;utm_campaign=itemfeed-all-pag-st-products-conv&amp;".($categNum>0?"utm_content=".$categNum."&amp;":'')."utm_term=".$arAcc["ID"]."</url>\n";
...
/*							$minPrice = $arPrice['DISCOUNT_PRICE'];
				...
				if (!empty($arCurrentOffer) && !empty($arCurrentPrice))
				{
					...
					$strOfferYandex .= "<url>".$usedProtocol.$ar_iblock['SERVER_NAME'].htmlspecialcharsbx($arOfferItem["~DETAIL_PAGE_URL"]).(strstr($arOfferItem['DETAIL_PAGE_URL'], '?') === false ? '?' : '&amp;')."utm_source=price_ru&amp;utm_medium=click&amp;utm_campaign=itemfeed-all-pag-st-products-conv&amp;utm_term=".$arOfferItem["ID"]."</url>\n";
...
/*						$minPrice = $arPrice['DISCOUNT_PRICE'];
				...
				$strOfferYandex.= "<url>".$usedProtocol.$ar_iblock['SERVER_NAME'].htmlspecialcharsbx($arItem["~DETAIL_PAGE_URL"]).(strstr($arItem['DETAIL_PAGE_URL'], '?') === false ? '?' : '&amp;')."utm_source=price_ru&amp;utm_medium=click&amp;utm_campaign=itemfeed-all-pag-st-products-conv&amp;utm_term=".$arItem["ID"]."</url>\n";
