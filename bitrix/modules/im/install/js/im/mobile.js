...
BX.ImMobile = function(params)
{
	...
	this.revision = 3; // mobile api revision - check include.php
	this.errorMessage = '';
...
BX.ImMessengerMobile.prototype.dialogStatusRedrawDelay = function(params)
{
	...
	if (this.openChatFlag)
	{
		...
		if (this.chat[chatId] && this.chat[chatId].type != 'call')
		{
			...
			app.menuCreate({items:[
				...
				{ icon: 'user', name: BX.message('IM_M_MENU_USERS'), action:BX.delegate(function() {
					BXMobileApp.PageManager.loadPageUnique({
	...
	if (BXMobileApp.UI.Page.TextPanel.params._plusAction)
	{
		BXMobileApp.UI.Page.TextPanel.params.plusAction = BXMobileApp.UI.Page.TextPanel.params._plusAction;
		delete BXMobileApp.UI.Page.TextPanel.params._plusAction;
		BXMobileApp.UI.Page.TextPanel.redraw();
	}
	...
	if (!params.slidingPanelRedrawDisable && (userId > 0 && !this.users[userId].fake || chatId > 0 && !this.chat[chatId].fake))
	{
		...
		else if (this.webrtc.mobileSupport && app.enableInVersion(10) && !this.users[userId].network)
		{
			showSlidingPanel = true;
		...
		else if (this.webrtc.mobileSupport && app.enableInVersion(9) && !this.users[userId].network)
		{
			showSlidingPanel = true;
		...
		else if (this.users[userId].network)
		{
			BXMobileApp.UI.Page.TextPanel.params._plusAction = BXMobileApp.UI.Page.TextPanel.params.plusAction;
			BXMobileApp.UI.Page.TextPanel.params.plusAction = window.platform == 'android'? "": "-1";
			BXMobileApp.UI.Page.TextPanel.redraw();
			app.hideButtonPanel();
		}
...
BX.ImMessengerMobile.prototype.openMessageMenu = function(messageId)
{
	...
	*/
	...
	{
		if (app.enableInVersion(14) && window.platform != 'android')
		{
			sheetButtons.push({
