...
if (check_bitrix_sessid())
{
	...
	else if ($_POST['IM_LOAD_LAST_MESSAGE'] == 'Y')
	{
		...
		if ($_POST['CHAT'] == 'Y')
		{
			...
			if (!$arMessage || $_POST['USER_LOAD'] == 'Y' && empty($arMessage['chat']) || isset($arMessage['chat'][$chatId]) && !in_array($arMessage['chat'][$chatId]['messageType'], Array(IM_MESSAGE_PUBLIC, IM_MESSAGE_CHAT)))
			{
				$arMessage = Array();
				$error = 'ACCESS_DENIED';
			}
	...
	else if ($_POST['IM_CONTACT_LIST_SEARCH'] == 'Y')
	{
		...
		else if (!IsModuleInstalled('intranet') && !IsModuleInstalled('b24network'))
		{
			$enabled = true;
		}
	...
	else if ($_POST['IM_RECENT_LIST'] == 'Y')
	{
		...
		{
			if ($value['TYPE'] == IM_MESSAGE_CHAT || $value['TYPE'] == IM_MESSAGE_PUBLIC)
			{
	...
	else if ($_POST['IM_CHAT_DATA_LOAD'] == 'Y')
	{
		...
		$arUser = CIMContactList::GetUserData(Array(
			'ID' => $arChat['userInChat'][$_POST['CHAT_ID']]
		));
