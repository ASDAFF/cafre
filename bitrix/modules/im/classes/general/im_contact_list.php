...
class CAllIMContactList
{
		...
		else if (isset($arParams['TO_CHAT_ID']))
		{
			...
			$strSql = "
				...
				WHERE R.USER_ID = ".$fromUserId."
					AND R.MESSAGE_TYPE IN ('".IM_MESSAGE_CHAT."', '".IM_MESSAGE_PUBLIC."')
	...
	public static function DeleteRecent($entityId, $isChat = false, $userId = false)
	{
		...
		$strSQL = "DELETE FROM b_im_recent WHERE USER_ID = ".$userId." AND ITEM_TYPE = '".($isChat? IM_MESSAGE_CHAT: IM_MESSAGE_PRIVATE)."' AND ".$sqlEntityId;
	...
	public static function GetRecentList($arParams = Array())
	{
		...
		$cache_dir = '/bx/imc/recent'.CIMMessenger::GetCachePath($GLOBALS['USER']->GetID());
		$obCache = new CPHPCache();
		if($obCache->InitCache($cache_ttl, $cache_id, $cache_dir))
		{
			$ar = $obCache->GetVars();
		...
		else
		{
			...
			while ($arRes = $dbRes->GetNext(true, false))
			{
				...
				if ($arRes['ITEM_TYPE'] == IM_MESSAGE_PUBLIC)
				{
				...
				else
				{
					...
					$chatType = $arRes["ITEM_TYPE"] == IM_MESSAGE_PUBLIC? 'public': 'chat';
			...
			if (!empty($toDelete))
			{
				...
				if (isset($toDelete[IM_MESSAGE_PUBLIC]))
					self::DeleteRecent($toDelete[IM_MESSAGE_PUBLIC], true);
			}
