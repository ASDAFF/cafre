...
class CIMChat
{
	...
	public function GetMessage($ID)
	{
		...
		{
			if ($arRes['CHAT_TYPE'] == IM_MESSAGE_PUBLIC)
			{
	...
	function GetLastMessage($toChatId, $fromUserId = false, $loadExtraData = false, $bTimeZone = true, $limit = true)
	{
		...
		if ($chatData['TYPE'] == IM_MESSAGE_PUBLIC)
		{
			$strSql = "
				SELECT
					M.ID,
					...
					R.USER_ID RID
				FROM b_im_message M
				INNER JOIN b_im_chat C ON C.ID = M.CHAT_ID AND C.TYPE = '".IM_MESSAGE_PUBLIC."'
				LEFT JOIN b_im_relation R ON R.CHAT_ID = M.CHAT_ID AND R.USER_ID = ".$this->user_id."
				WHERE
					M.CHAT_ID = ".$toChatId."
					".$limitById."
		...
		if ($chatType == IM_MESSAGE_PUBLIC)
		{
			if ($chatRelationUserId <= 0 && IM\User::getInstance($this->userId)->isExtranet())
			{
				$arMessages = Array();
				$arMessageId = Array();
				$arUsersMessage = Array();
		...
		if (is_array($loadExtraData) || is_bool($loadExtraData) && $loadExtraData == true)
		{
			...
			if ($arChat['chat'][$toChatId]['messageType'] == IM_MESSAGE_PUBLIC || in_array($fromUserId, $arChat['userInChat'][$toChatId]))
			{
	...
	public function GetLastSendMessage($arParams)
	{
		...
		while ($arRes = $dbRes->Fetch())
		{
			...
			{
				$chatType = $arRes["CHAT_TYPE"] == IM_MESSAGE_PUBLIC? 'public': 'chat';
			}
	...
	public static function GetChatData($arParams = Array())
	{
		...
		{
			$from .= " AND C.TYPE IN ('".IM_MESSAGE_PUBLIC."','".IM_MESSAGE_CHAT."')";
		}
	...
	public function GetUnreadMessage($arParams = Array())
	{
		...
		$messageType = isset($arParams['MESSAGE_TYPE']) && in_array($arParams['MESSAGE_TYPE'], Array(IM_MESSAGE_PUBLIC, IM_MESSAGE_CHAT))? $arParams['MESSAGE_TYPE']: 'ALL';
			...
			if (strlen($ssqlStatus) > 0)
			{
				$strSql ="
					...
					WHERE
						...
						".($messageType == 'ALL'? "AND R1.MESSAGE_TYPE IN ('".IM_MESSAGE_PUBLIC."','".IM_MESSAGE_CHAT."')": "AND R1.MESSAGE_TYPE = '".$messageType."'")."
						".$ssqlStatus."
				";
	...
	public function SetColor($chatId, $color)
	{
		...
		$strSql = "
			...
			WHERE R.USER_ID = ".$this->user_id." AND R.MESSAGE_TYPE IN ('".IM_MESSAGE_PUBLIC."','".IM_MESSAGE_CHAT."') AND R.CHAT_ID = ".$chatId;
		$dbRes = $DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);
		if ($arRes = $dbRes->Fetch())
		{
	...
	public function Add($arParams)
	{
		...
		$messageType = IM_MESSAGE_CHAT;
		if (isset($arParams['MESSAGE_TYPE']) && in_array($arParams['MESSAGE_TYPE'], Array(IM_MESSAGE_PUBLIC, IM_MESSAGE_CHAT)))
			$messageType = $arParams['MESSAGE_TYPE'];
	...
	{
		$arFields['MESSAGE_TYPE'] = isset($arParams['MESSAGE_TYPE']) && in_array($arParams['MESSAGE_TYPE'], Array(IM_MESSAGE_PUBLIC, IM_MESSAGE_CHAT))? $arParams['MESSAGE_TYPE']: IM_MESSAGE_CHAT;
	...
	public function AddUser($chatId, $userId)
	{
		...
		$strSql = "
			...
			WHERE
				...
				R.MESSAGE_TYPE IN ('".IM_MESSAGE_PUBLIC."','".IM_MESSAGE_CHAT."')
		...
		if ($arRes = $dbRes->Fetch())
		{
			...
			else
			{
				...
				{
					$message = GetMessage("IM_CHAT_SELF_JOIN_".$arUsers[$arUserId[0]]['gender'], Array('#USER_NAME#' => implode(', ', $arUsersName)));
				}
				else
				{
			...
			foreach ($arUserId as $userId)
			{
				...
				$orm = IM\RelationTable::add(array(
					...
					"USER_ID" => $userId,
					"START_ID" => CIMSettings::GetStartChatMessage() == CIMSettings::START_MESSAGE_LAST && $arRes['CHAT_TYPE'] == IM_MESSAGE_CHAT? $maxId+1: 0,
					"LAST_ID" => $maxId,
					"LAST_SEND_ID" => $maxId,
					"LAST_FILE_ID" => CIMSettings::GetStartChatMessage() == CIMSettings::START_MESSAGE_LAST && $arRes['CHAT_TYPE'] == IM_MESSAGE_CHAT? $fileMaxId: 0,
				));
				$update[] = $orm->getId();
	...
	public function DeleteUser($chatId, $userId, $checkPermission = true)
	{
		...
		$strSql = "
			SELECT R.CHAT_ID, C.TITLE CHAT_TITLE, C.AUTHOR_ID CHAT_AUTHOR_ID, C.EXTRANET CHAT_EXTRANET
			FROM b_im_relation R LEFT JOIN b_im_chat C ON R.CHAT_ID = C.ID
			WHERE R.USER_ID = ".$userId." AND R.MESSAGE_TYPE IN ('".IM_MESSAGE_PUBLIC."','".IM_MESSAGE_CHAT."') AND R.CHAT_ID = ".$chatId;
		$dbRes = $DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);
		if ($arRes = $dbRes->Fetch())
		{
			$extranetFlag = $arRes["CHAT_EXTRANET"] == ""? "": ($arRes["CHAT_EXTRANET"] == "Y"? true: false);
	...
	public static function SetUnreadCounter($userId)
	{
		...
		$sqlCounter = "SELECT COUNT(M.ID) as CNT
						FROM b_im_message M
						INNER JOIN b_im_relation R1 ON M.ID > R1.LAST_ID AND M.CHAT_ID = R1.CHAT_ID AND R1.MESSAGE_TYPE IN ('".IM_MESSAGE_PUBLIC."','".IM_MESSAGE_CHAT."') AND R1.STATUS < ".IM_STATUS_READ."
