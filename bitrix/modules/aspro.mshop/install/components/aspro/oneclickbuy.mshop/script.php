...
if(!$bAllBasketBuy && !$USER->IsAuthorized()){
	$resBasketItems = CSaleBasket::GetList(array('SORT' => 'DESC'), array('FUSER_ID' => $basketUserID, 'LID' => SITE_ID, 'ORDER_ID' => NULL), false, false, array('ID', 'QUANTITY', 'PRODUCT_ID', 'TYPE', 'SET_PARENT_ID'));
...
if($bAllBasketBuy){
	...
	$productQuantity = (intVal($_POST['ELEMENT_QUANTITY']) > 0 ? intVal($_POST['ELEMENT_QUANTITY']) : 1);

	$resProduct = CIBlockElement::GetByID($productID);
	$arProduct = $resProduct->GetNext();

	if(strlen($_REQUEST['OFFER_PROPERTIES']) && $iblockID > 0){
		$arOfferProperties = explode(';', $_REQUEST['OFFER_PROPERTIES']);
...
if($orderID){
	...
	$arMessageFields = array(
		...
		"ORDER_PRICE" => str_replace('#', number_format($orderPrice, $arCurrency["DECIMALS"], $arCurrency["DEC_POINT"], $currencyThousandsSep), $arCurrency['FORMAT_STRING']),
